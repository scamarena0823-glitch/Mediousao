...
(function(){
 ...
 async function showItems(orderId){
  const m=modal('Productos del pedido','<div style="padding:18px 0;color:#777">Cargando productos…</div>');const body=m.querySelector('#mediousaoOrderProductsBody');
  try{
   const iq=await client.from('order_items').select('product_id,product_name,quantity,price').eq('order_id',orderId);if(iq.error)throw iq.error;
   ...
   const ids=[...new Set(items.map(x=>x.product_id).filter(Boolean))],products={};
  ...
  }catch(e){console.error('MEDIOUSAO order items',e);body.innerHTML='<div style="padding:14px;border-radius:12px;background:#fff0f0;color:#a00000">No se pudieron cargar los productos del pedido.</div>'}
 }
 /* El index llama directamente viewOrderItems(orderId). Sustituimos esa función antigua. */
 window.viewOrderItems=showItems;
})();