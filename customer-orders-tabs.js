...
(function(){
 ...
 function install(){
  ...
  if((box.textContent||'').toLowerCase().includes('cargando pedidos')){timer=setTimeout(install,180);return}
  if(box.querySelector('#mediousaoOrderTabs'))return;
  const cards=[...box.children].filter(c=>orderNo(c));
  if(!cards.length && !(box.textContent||'').toLowerCase().includes('aún no tienes pedidos')){timer=setTimeout(install,180);return}
 ...
 function schedule(){clearTimeout(timer);timer=setTimeout(install,520)}
 document.addEventListener('click',e=>{if(e.target.closest('[data-go="orders"]'))schedule()},true);
 window.addEventListener('mediousao:orders-rendered',schedule);
 const mo=new MutationObserver(m=>{const box=document.getElementById('profileOrders');if(!box)return;const relevant=m.some(x=>x.target===box||box.contains(x.target));if(relevant&&!box.querySelector('#mediousaoOrderTabs'))schedule()});
