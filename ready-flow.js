...
(function(){
  ...
  function pagination(total){
    ...
    box.querySelector('[data-page="next"]').onclick=()=>{if(currentPage<pages){currentPage++;render();window.scrollTo({top:0,behavior:'smooth'})}};
  }
  window.render=function(){
    ...
    const pages=Math.max(1,Math.ceil(arr.length/PAGE_SIZE)); if(currentPage>pages)currentPage=pages;
    ...
    const grid=document.getElementById('grid'); if(grid)grid.innerHTML=pageItems.map(cardFixed).join('')||"<div class='notice'>No encontramos productos.</div>";
    pagination(arr.length);
  ...
  function addPaginationBox(){
    if(document.getElementById('mediousaoPagination'))return;
    const grid=document.getElementById('grid'); if(!grid||!grid.parentNode)return;
    const box=document.createElement('div'); box.id='mediousaoPagination'; grid.parentNode.insertBefore(box,grid.nextSibling);
  }
  function cleanHome(){
    removeSearch(); addTopCart(); addBottomNav(); addPaginationBox();
