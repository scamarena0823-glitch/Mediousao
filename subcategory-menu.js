...
(function(){
  ...
  window.addSubcategory=async function(){const cat=document.getElementById('subcatCategory')?.value,name=document.getElementById('subcatName')?.value.trim();if(!cat||!name)return msg('Selecciona categoría y escribe el nombre.',true);try{const list=await rows(cat);const {error}=await client.from('subcategories').insert({category_slug:cat,name,slug:slugify(name),active:true,sort_order:list.length+1});if(error)throw error;document.getElementById('subcatName').value='';msg('Opción agregada. ✅');loadAdmin();window.refreshProductSubcategories?.();}catch(e){console.error(e);msg('No se pudo agregar.',true);}};
  window.editSubcategory=async function(id,old){const name=prompt('Nuevo nombre',old);if(!name||name===old)return;const {error}=await client.from('subcategories').update({name,slug:slugify(name)}).eq('id',id);if(error)return msg('No se pudo editar.',true);loadAdmin();window.refreshProductSubcategories?.();};
  window.toggleSubcategory=async function(id,val){const {error}=await client.from('subcategories').update({active:val}).eq('id',id);if(error)return msg('No se pudo cambiar.',true);loadAdmin();window.refreshProductSubcategories?.();};
  window.deleteSubcategory=async function(id){if(!confirm('¿Eliminar esta opción del menú?'))return;const {error}=await client.from('subcategories').delete().eq('id',id);if(error)return msg('No se pudo eliminar.',true);loadAdmin();window.refreshProductSubcategories?.();};
  ...
  function installProductField(){
    ...
    let s=document.getElementById('pSubcategory');if(!s){s=document.createElement('select');s.id='pSubcategory';s.className='search';s.style.marginBottom='8px';hidden.insertAdjacentElement('afterend',s);}
    async function refresh(){
      const categorySelect=document.getElementById('productCategorySelect');
      const cat=(categorySelect&&categorySelect.value)||hidden.value||'';
      if(!cat){s.innerHTML='<option value="">Tipo (opcional)</option>';return;}
      try{const list=await active(cat);const keep=s.value;s.innerHTML='<option value="">Tipo (opcional)</option>'+list.map(x=>`<option value="${esc(x.slug)}">${esc(x.name)}</option>`).join('');if([...s.options].some(o=>o.value===keep))s.value=keep;}catch(e){console.error(e);s.innerHTML='<option value="">Tipo (opcional)</option>';}
    }
    window.refreshProductSubcategories=refresh;
    document.addEventListener('change',e=>{if(e.target.id==='productCategorySelect'){hidden.value=e.target.value;refresh();}});
